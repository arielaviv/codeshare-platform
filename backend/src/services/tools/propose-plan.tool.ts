...
export const proposePlanTool: AgentTool = {
  ...
  async execute(input: unknown, ctx: ToolContext): Promise<string> {
    ...
    const { plan, pricing } = await scopePlan(userPrompt, ctx.userId ?? undefined);
    ...
    const anchor = formatUsd(pricing.anchorCents);
    const dealer = formatUsd(pricing.dealerCents);

    return [
      `Plan "${plan.title}" proposed (id=${plan.id}).`,
      `Pricing: ${anchor} anchor → ${dealer} dealer · tier=${pricing.tier} · ~${pricing.estimatedTurns} turns.`,
      '',
      'The user now sees a plan-approval widget. DO NOT write any files yet.',
      'Respond with ONE short sentence acknowledging the plan, then stop and wait for the user.',
      'The user will reply with one of:',
      '- "Accept the plan. mode=auto clearContext=false" → begin building immediately.',
      '- "Accept the plan. mode=ask clearContext=false" → build but pause before each write (treat as auto for now).',
