...
export default function PreviewPanel({ files, isGenerating, fallbackHtml }: PreviewPanelProps) {
  ...
  if (useWC && wcState.status === 'running' && wcState.url) {
    ...
    return (
      <div className="h-full flex flex-col bg-[#1A1A1A]">
        <div className="flex items-center justify-center gap-1 px-3 py-1.5 border-b border-[#2A2A2A] flex-shrink-0">
          {(['desktop', 'tablet', 'phone'] as const).map((d) => (
            <button
              key={d}
              onClick={() => setDevice(d)}
              className={`p-1.5 rounded transition-colors ${device === d ? 'text-[#E8E8E8] bg-[#333]' : 'text-[#666] hover:text-[#A0A0A0]'}`}
          ...
          <button onClick={refresh} className="p-1.5 text-[#666] hover:text-[#A0A0A0] transition-colors">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <polyline points="23 4 23 10 17 10" /><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" />
            </svg>
          </button>
        </div>
        <div className="flex-1 flex items-start justify-center overflow-auto bg-[#111] p-2">
          <iframe
            src={wcState.url}
        ...
        <div className="flex items-center justify-between px-3 py-1 bg-[#1A1A1A] border-t border-[#2A2A2A] text-[10px] text-[#666] flex-shrink-0">
  ...
  if (useWC && !['idle', 'error'].includes(wcState.status)) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-[#0A0A0A] gap-4">
        ...
        <div className="w-24 h-0.5 bg-[#222] rounded-full overflow-hidden">
          <div className="h-full w-8 bg-[#666] rounded-full animate-progress-sweep" />
        </div>
      </div>
    );
  }
  ...
  if (useWC && wcState.status === 'error') {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-[#0A0A0A] gap-3">
        <span className="text-sm text-[#A0A0A0]">Preview error</span>
        <span className="text-xs text-[#666] max-w-xs text-center">{wcState.error}</span>
        <button onClick={refresh} className="text-xs text-[#A0A0A0] hover:text-white border border-[#333] px-3 py-1 rounded transition-colors">Retry</button>
      </div>
    );
  }
  ...
  if (isGenerating) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-[#0A0A0A] gap-4">
        <span className="w-2 h-2 rounded-full bg-[#FFAB00] animate-pulse" />
        <span className="text-sm text-[#A0A0A0]">Building...</span>
      </div>
    );
  }

  return (
    <div className="h-full flex items-center justify-center bg-[#0A0A0A] text-[#444] text-sm">
